    ...
    <section class="panel">
        <form class="enroll-form" method="post" action="{{ route('admin.students.enroll') }}" enctype="multipart/form-data">
            ...
            <label class="field">الطالب
                <select name="student_id" id="enroll-student-select" required><option value="">اختر الطالب</option>@foreach($enrollmentStudents as $student)<option value="{{ $student->id }}" data-grade-level="{{ $student->grade_level }}" data-market-id="{{ $student->market_id }}" data-enrolled="{{ implode(',', $studentEnrollments->get($student->id, [])) }}">{{ $student->full_name }}{{ $student->phone ? ' - '.$student->phone : '' }}</option>@endforeach</select>
            </label>
            <div class="field"><div class="subjects-heading"><h2>المواد</h2><span class="subject-count" id="selected-count">0 مواد محددة</span></div><p class="empty-state" id="select-student-hint">اختر طالبًا أولًا لعرض المواد المتاحة لصفه ودولته.</p>
            <div class="subjects-grid" id="subjects-grid">@forelse($subjects as $subject)<label class="subject-option" data-grade-id="{{ $subject->grade_id }}" data-market-id="{{ $subject->market_id }}" hidden><input type="checkbox" name="subject_ids[]" value="{{ $subject->id }}"><span>{{ $subject->name }}<small>{{ $subject->grade_name }}</small></span></label>@empty<p class="empty-state">لا توجد مواد منشورة.</p>@endforelse</div></div>
...
<script>
    ...
    const studentSelect=document.querySelector('#enroll-student-select');
    const subjectLabels=document.querySelectorAll('#subjects-grid .subject-option');
    const hint=document.querySelector('#select-student-hint');

    function updateCount(){
        const selected=document.querySelectorAll('input[name="subject_ids[]"]:checked').length;
        count.textContent=selected+' مواد محددة';
    }
    ...
    function filterSubjectsByStudent(){
        ...
        const gradeLevel=hasStudent?(option.dataset.gradeLevel||''):'';
        const marketId=hasStudent?(option.dataset.marketId||''):'';
        const gradeId=gradeLevel&&gradeLevelToId[gradeLevel]?String(gradeLevelToId[gradeLevel]):'';
        const enrolledIds=hasStudent&&option.dataset.enrolled?option.dataset.enrolled.split(',').filter(Boolean):[];

        hint.hidden=hasStudent;

        subjectLabels.forEach(function(label){
            const box=label.querySelector('input[type="checkbox"]');
            const alreadyEnrolled=enrolledIds.includes(box.value);
            const matches=hasStudent&&gradeId!==''&&label.dataset.gradeId===gradeId&&label.dataset.marketId===marketId&&!alreadyEnrolled;
            label.hidden=!matches;
            if(!matches&&box.checked){box.checked=false}
        });
    ...
    subjectLabels.forEach(function(label){
        label.querySelector('input[type="checkbox"]').addEventListener('change',updateCount);
    });
    studentSelect.addEventListener('change',filterSubjectsByStudent);
