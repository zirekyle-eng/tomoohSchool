...
class AdminApplicationsController extends Controller
{
    ...
    {
        $application = DB::table('teacher_applications')->where('id', $id)->where('status', 'pending')->first();
    ...
    {
        DB::table('teacher_applications')->where('id', $id)->where('status', 'pending')->update(['status' => 'rejected', 'updated_at' => now()]);
